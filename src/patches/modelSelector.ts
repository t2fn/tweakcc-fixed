...
const injectSettingsReader = (fileContents: string): string | null => {
  ...
  const readerFunc = `globalThis.__tweakccReadSettings=function(){var m=globalThis.__tweakccCustomModels;if(!m)m=[];try{var f=require("fs");var cs=[];var sep="/";if(process.env.CLAUDE_CONFIG_DIR)cs.push(process.env.CLAUDE_CONFIG_DIR.replace(/[\\/]+$/,"")+sep+"settings.json");var hd=process.env.HOME||process.env.USERPROFILE;if(hd)cs.push(hd.replace(/[\\/]+$/,"")+sep+".claude"+sep+"settings.json");for(var pi=0;pi<cs.length;pi++){try{var d=JSON.parse(f.readFileSync(cs[pi],"utf8"));if(d.customModels)for(var x of d.customModels){var already=m.some(function(e){return e.value===x.value});if(!already)m.push({value:x.value,label:x.label||x.value,description:"",contextWindow:x.contextWindow||200000,maxTokens:x.maxTokens||16384,subModels:x.subModels&&x.subModels.haiku?{haiku:String(x.subModels.haiku)}:void 0,compactThresholdPct:+x.compactThresholdPct>0&&+x.compactThresholdPct<=100?+x.compactThresholdPct:void 0})}var v=d&&d.modelOverrides;for(var k in v){var vals=v[k];if(Array.isArray(vals)){var ex=null;for(var q=0;q<m.length;q++){if(m[q].value===k){ex=m[q];break}}var nw=null;for(var j=0;j<vals.length;j++){var parts=String(vals[j]).split("=");if(parts[0]==="contextWindow"||parts[0]==="haiku"||parts[0]==="compactThresholdPct"){if(!ex&&!nw)nw={value:k,label:k,description:"",contextWindow:200000,maxTokens:16384};var tgt=ex||nw;if(parts[0]==="contextWindow"){tgt.contextWindow=Number(parts[1])||200000}else if(parts[0]==="compactThresholdPct"){var pn=Number(parts[1]);if(pn>0&&pn<=100)tgt.compactThresholdPct=pn}else if(parts[1]&&!(tgt.subModels&&tgt.subModels.haiku)){tgt.subModels={haiku:parts[1]}}}}if(nw)m.push(nw)}}}catch(u){if(process.env.TWEAKCC_DEBUG)console.error("tweakcc reader: "+cs[pi]+": "+(u&&u.message))}}}catch(t){if(process.env.TWEAKCC_DEBUG)console.error("tweakcc reader: "+(t&&t.message))}globalThis.__tweakccCustomModels=m};
